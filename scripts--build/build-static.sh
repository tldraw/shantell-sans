...
fontmake -o ttf -i -m $DS --expand-features-to-instances -i '.*Informal SemiBold.*' --output-dir "$outputDir/static-TTF" &                   # use this line for a much faster test build of onlyInformal ExtraBold 
wait

# find "sources/build-prep/ital_wght_BNCE_INFM_SPAC--prepped/instances" -path '*.ufo' -print0 | while read -d $'\0' file
# do
#     fontmake -o otf -u "$file" --optimize-cff=0 -i ".*Informal ExtraBold" --output-dir "$outputDir/static-OTF" &
#     wait
# done
